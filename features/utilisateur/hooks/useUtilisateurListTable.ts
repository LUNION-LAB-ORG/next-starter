...
export function useUtilisateurListTable({
  ...
  const [filters, setFilters] = useQueryStates(
    utilisateurFiltersClient.filter,
    utilisateurFiltersClient.option,
  );
  ...
  const { data, isLoading, isError, isFetching } =
    useUtilisateursListQuery(currentSearchParams);

  console.log("Utilisateur List Data:", data);
  ...
  const handleEnumFilterChange = useCallback(
    (filterName: "status" | "role", value: string) => {
      setFilters((prev) => ({
        ...prev,
        [filterName]: value,
        page: 1,
